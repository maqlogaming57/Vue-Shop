...
const reduceQuantity = async (item) => {
    ...
    try {
        await router.patch(route('cart.update', item.product_id), {
            ...
            size: item.size || '',
            color: item.color || '',
            note: item.note || '',  // Make sure to include note to preserve it
            onSuccess: page => {
                ElNotification({
                    title: 'Success',
...
// Fixed updateVariant function with proper error handling and debug logging
const updateVariant = async (item) => {
    try {
        console.log('Updating variant with note:', item.note); // Debug log
        ...
        await router.patch(route('cart.update', item.product_id), {
            ...
            onSuccess: page => {
                ...
                console.log('Update successful:', page.props.flash.success); // Debug log
            },
            onError: errors => {
...
// Improved watch function to properly initialize and debug item notes
watch(
    () => carts.value,
    (newCarts) => {
        if (newCarts) {
            newCarts.forEach(item => {
                // If note is undefined, null, or not a string, initialize it properly
                if (!item.note) {
                    item.note = '';
                    console.log(`Initialized empty note for item ${item.product_id}`);
                } else {
                    console.log(`Item ${item.product_id} has note: ${item.note}`);
                }
            ...
            <div class="container px-5 pt-12 mx-auto h-screen flex sm:flex-nowrap flex-wrap gap-4">
                <div class="relative overflow-x-auto sm:rounded-lg lg:w-2/3 md:w-1/2 lg:px-6">
                    ...
                    <table class="w-full text-sm text-left overflow-y-scroll max-h-[800px] text-gray-500 dark:text-gray-400 rounded-lg">
                        ...
                        <tbody>
                        <template v-if="carts" v-for="(item, index) in carts" :key="index">
                            ...
                            <tr class="bg-gray-50 dark:bg-gray-900">
                                <td colspan="7" class="px-6 py-4">
                                    <label class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Catatan Tambahan / Link Desain</label>
                                    <textarea
                                    v-model="item.note"
                                    @change="updateVariant(item)"
                                    placeholder="Contoh: Tulis nama di bagian depan kaos. Link desain: https://..."
                                    class="w-full p-3 text-sm border border-gray-300 rounded-lg resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:placeholder-gray-400"
                                    rows="3"
                                    ></textarea>
                                </td>
                            </tr>
